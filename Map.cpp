...
void Map::create_maze(int sx, int sy, int ex, int ey)
{
...
bool Map::reachable(const int& delta_x, const int& delta_y)
...
bool Map::win(void) const
{
	return (x >= row - 2 and y >= col - 1) or (x >= row - 1 and y >= col - 2);
}

const char& Map::symbol(const int& _x, const int& _y) const
//返回迷宫对应坐标的char符号
{
...
void Map::color(void) const
{
	set_color_and_background(15, 0);
	return;
}

void Map::color(const int& _x, const int& _y) const
{
	if (_x <= 0 or _x >= row + 1 or _y <= 0 or _y >= col + 1) set_color_and_background(15, 0);
	else if (_x == x and _y == y) set_color_and_background(12, 0);
...
void Map::move(const int& delta_x, const int& delta_y)
...
void Map::print_with_fog(const int& length, const int& width) const
...
Map::Map(const int& _row, const int& _col)
{
	row = _row;
	col = _col;
	x = row / 2 + 1;
